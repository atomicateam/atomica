...
models = list()
for f in os.listdir(au.LIBRARY_PATH):
...
def run_reconciliation(proj):
    ...
    au.reconcile(project=proj, parset=proj.parsets[0], progset=proj.progsets[0], reconciliation_year=2018, unit_cost_bounds=0.05, baseline_bounds=0.05, capacity_bounds=0.05, outcome_bounds=0.05)
...
def run_budget_scenario(proj):
    ...
    scen = au.BudgetScenario(name='Doubled budget',alloc=doubled_budget,start_year=2018)
...
def run_coverage_scenario(proj):
    ...
    scen = au.CoverageScenario(name='Reduced coverage',coverage=half_coverage,start_year=2018)
    scen.run(proj,parset='default',progset='default')
...
def run_optimization(proj):
    ...
    instructions = au.ProgramInstructions(alloc=proj.progsets[0],start_year=2020) # Instructions for default spending
    adjustments = [au.SpendingAdjustment(x,2020,'rel',0.,2.) for x in instructions.alloc.keys()]
    measurables = au.MaximizeCascadeStage(None,2020)
    constraints = au.TotalSpendConstraint() # Cap total spending in all years
    optimization = au.Optimization(name='default', adjustments=adjustments, measurables=measurables,constraints=constraints,maxtime=10) # Evaluate from 2020 to end of simulation
    optimized_instructions = au.optimize(proj, optimization, parset=proj.parsets["default"], progset=proj.progsets['default'], instructions=instructions)
...
def test_model(model):
    ...
    framework_file = au.LIBRARY_PATH + model + '_framework.xlsx'
    databook_file = au.LIBRARY_PATH + model + '_databook.xlsx'
    progbook_file = au.LIBRARY_PATH + model + '_progbook.xlsx'

    print('\n\nTESTING %s project' % (model))

    P = au.Project(framework=framework_file)
    ...
    P.run_sim(P.parsets[0],P.progsets[0],au.ProgramInstructions(start_year=2018))
