...
def interpolate_func(x, y, xnew, method='pchip', extrapolate_nan=False):
    """
    Interpolates and extrapolates (x,y) coordinates along domain subset of xnew, according to chosen method.
    ...
    if not len(x) > 1:
        raise AtomicaException("ERROR: Interpolation failure due to the existence of less than two x values.")
    if not len(x) == len(y):
        raise AtomicaException("ERROR: Interpolation failure due to unequal number of x and y values.")
    if len(set(x)) != len(x):
        raise AtomicaException("ERROR: Interpolation failure due to repeated x values.")
    ...
    if method == 'pchip':
        ...
        ynew = pchip_eval(x, y, m, xnew,
                          deriv=False)  # Use these slopes (along with the Hermite basis function) to interpolate.
...
def pchip_slopes(x, y):
    ...
    for c in range(len(x)):
        if c == 0:
            ...
            deriv = secants[c - 1]
        elif secants[c] * secants[c - 1] <= 0:
            deriv = 0
        else:
            deriv = (secants[c] + secants[c - 1]) / 2

        if c < len(x) - 1 and abs(deriv) > 3 * abs(secants[c]):
            deriv = 3 * secants[c]
        if c > 0 and abs(deriv) > 3 * abs(secants[c - 1]):
            deriv = 3 * secants[c - 1]
