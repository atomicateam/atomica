...
import numpy as np
from .cascade import validate_cascade, InvalidCascade
    ...
    def _validate(self):
        ...
        for index,row in self.comps.iterrows():
            ...
            if (row['setup weight']>0) & (row['databook page'] is None):
                raise AtomicaException('Compartment "%s" has a nonzero setup weight, but does not appear in the databook' % row.name)
        ...
        for i,par in self.pars.iterrows():
            if self.transitions[par.name]: # If this parameter is associated with transitions
                ...
                if n_source_outflow > 1:
                    raise AtomicaException('Parameter "%s" has an outflow from more than one source compartment, which prevents disaggregation from working correctly' % par.name)
