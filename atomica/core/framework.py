...
class ProjectFramework(object):
    ...
    def new(num_comps=0, num_characs=0, num_pars=0, num_datapages=0, num_interpops=0):
        ...
        template_file =  os.path.abspath(os.path.join(atomica_path('core'),'framework_template.xlsx'))
        ...
        sheets['Characteristics'] = pd.DataFrame(index=None,columns=['Code Name','Display Name','Components','Denominator','Default Value','Setup Weight','Can Calibrate','Databook Page','Databook Order'])
        sheets['Interactions'] = pd.DataFrame(index=None,columns=['Code Name','Display Name','Default Value'])
        sheets['Parameters'] = pd.DataFrame(index=None,columns=['Code Name','Display Name','Format','Default Value','Minimum Value','Maximum Value','Function','Is Impact','Can Calibrate','Databook Page','Databook Order'])
        ...
        for i in range(0, num_pars):
            sheets['Parameters'].loc[i] = ['par_%d' % i, 'Parameter %d' % i, None,None,None,None,'n','n',None,None,None]
        ...
        sheets['Interactions'].set_index('Code Name', inplace=True)
