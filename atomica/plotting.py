                ...
                for output_name in aggregated_outputs[list(aggregated_outputs.keys())[0]].keys():
                    if isinstance(pop, dict):
                        ...
                        if aggregation == "sum":
                            ...
                            vals = np.divide(numerator, denominator, out=np.full(numerator.shape, np.nan, dtype=float), where=denominator != 0)  ## TODO: Check with Romesh when merging stochastic_transtions branch
                        else:
