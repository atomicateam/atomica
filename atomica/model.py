...
class Compartment(Variable):
    ...
    def __init__(self, pop, name, multinomial: bool = False, rng_sampler=None):
        ...
        if self.multinomial and rng_sampler is None:
...
class JunctionCompartment(Compartment):
    def __init__(self, pop, name: str, duration_group: str = None, multinomial: bool = False, rng_sampler=None):
        """
        ...
        """
        super().__init__(pop, name, multinomial=multinomial, rng_sampler=rng_sampler)
    ...
    def balance(self, ti: int) -> None:
        ...
        if self.multinomial and net_inflow > 0.0:
            if self.duration_group:
...
class ResidualJunctionCompartment(JunctionCompartment):
    ...
    def balance(self, ti: int) -> None:
        ...
        for frac, link in zip(outflow_fractions, self.outlinks):
            if link.parameter is None and total_outflow < 1 and not self.multinomial:  # If we are stochastic then we have already taken the residual link into account
    ...
    def initial_flush(self) -> None:
        ...
        if self.vals[0] > 0:
            ...
            # assign outflow stochastically to compartments based on the probabilities
            if self.multinomial and total_outflow > 0.0:
...
class TimedCompartment(Compartment):
    def __init__(self, pop, name: str, parameter: ParsetParameter, multinomial: bool = False, rng_sampler=None):
        """
        ...
        Compartment.__init__(self, pop=pop, name=name, multinomial=multinomial, rng_sampler=rng_sampler)
    ...
    def resolve_outflows(self, ti: int) -> None:
        ...
        assert self.multinomial == False, "resolve_outflows needs to be updated to work with stochastic transitions"
...
class Population:
    ...
    def __init__(self, framework, name: str, label: str, progset: ProgramSet, pop_type: str, multinomial: bool = False, rng_sampler=None, build_cache: dict | None = None):
        """
        ...
        :param multinomial: whether to use stochasticity in determining population movement
        ...
        self.multinomial = multinomial  #: Whether the population should be handled stochastically as discrete integer people instead of fractions
        self.rng_sampler = rng_sampler
        ...
        for comp_name in list(comps.index):
            ...
            if population_type == self.type:
                if comp_name in residual_junctions:
                    self.comps.append(ResidualJunctionCompartment(pop=self, name=comp_name, multinomial=self.multinomial, rng_sampler=self.rng_sampler, duration_group=duration_group))
                elif is_junction == "y":
                    self.comps.append(JunctionCompartment(pop=self, name=comp_name, multinomial=self.multinomial, rng_sampler=self.rng_sampler, duration_group=duration_group))
                elif duration_group:
                    self.comps.append(TimedCompartment(pop=self, name=comp_name, multinomial=self.multinomial, rng_sampler=self.rng_sampler, parameter=self.par_lookup[duration_group]))
                elif is_source == "y":
                    self.comps.append(SourceCompartment(pop=self, name=comp_name, multinomial=self.multinomial, rng_sampler=self.rng_sampler))
                elif is_sink == "y":
                    self.comps.append(SinkCompartment(pop=self, name=comp_name, multinomial=self.multinomial, rng_sampler=self.rng_sampler))
                else:
                    self.comps.append(Compartment(pop=self, name=comp_name, multinomial=self.multinomial, rng_sampler=self.rng_sampler))
    ...
    def initialize_compartments(self, parset: ParameterSet, framework, t_init: float) -> None:
        ...
        for i, c in enumerate(comps):
            if self.multinomial:
                c[0] = stochastic_rounding(max(0.0, x[i,0]), rng_sampler=self.rng_sampler)  # integer values (still represented as floats)
            else:
                c[0] = max(0.0, x[i,0])
...
class Model:
    ...
    def __init__(self, settings, framework, parset, progset=None, program_instructions=None, rng_sampler=None, acceptance_criteria=[]):
        ...
        self.multinomial = settings.multinomial  #: Run with discrete numbers of people per compartment instead of fractions.
    ...
    def build(self, parset):
        ...
        for k, (pop_name, pop_label, pop_type) in enumerate(zip(parset.pop_names, parset.pop_labels, parset.pop_types)):
            self.pops.append(Population(framework=self.framework, name=pop_name, label=pop_label, progset=self.progset, pop_type=pop_type, multinomial=self.multinomial, rng_sampler=self.rng_sampler, build_cache=build_cache))
    ...
    def update_pars(self) -> None:
        ...
        for par_name in self._exec_order["dynamic_pars"]:
            ...
            pars = self._vars_by_pop[par_name] # The parameter instances being updated
            ...
            if pars[0].pop_aggregation:
                ...
                agg_fcn = pars[0].pop_aggregation[0] # The aggregation function e.g., "SRC_POP_AVG"
                agg_vars = self._vars_by_pop[pars[0].pop_aggregation[1]] # The variable instances being aggregated
                interactions = self.interactions[pars[0].pop_aggregation[2]] if len(pars[0].pop_aggregation) > 2 else None
                weighting_vars = self._vars_by_pop[pars[0].pop_aggregation[3]] if len(pars[0].pop_aggregation) > 3 else None

                if interactions is None:
                    # No interaction term and no weighting variable - no matrix multiplication required
                    # Every output parameter gets the same value (sum or average of the agg_vars)
                    val = sum(x[ti] for x in agg_vars)

                    if agg_fcn in {"SRC_POP_AVG", "TGT_POP_AVG"} and agg_vars:
                        val /= len(agg_vars)
                ...
                elif interactions.shape[0] == 1 and interactions.shape[1] == 1:
                    ...
                    weight = interactions[0, 0, ti] # On this branch, we know that the interaction must exist
                    if weighting_vars:
                        weight = weight * weighting_vars[0][ti]  # Multiply by the weighting variable

                    # Since there is only one term, the _AVG functions will have the weight cancel out, unless it is zero
                    # Therefore they do not need to be explicitly handled here
                    if weight == 0:
                        val = 0
                    ...
                    par_vals = [x[ti] for x in agg_vars]  # Value of variable being averaged
                    ...
                    if weighting_vars:
                        vals = [par[ti] for par in weighting_vars]  # Value of weighting variable
